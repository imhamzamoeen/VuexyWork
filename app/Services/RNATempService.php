        ...
        foreach ($company_data->policy as $sub_policy) {
            ...
            if (isset($sub_policy->policyrates[0]->age)) {
                ...
                if ($rate_from_db > 0) {
                    ...
                    $monthly = (round(($rate_from_db * $monthly_mode_factor), 2) * ($coverage / 1000)) + $monthly_policy_fee;


                    $annual = (round(($rate_from_db * $annual_mode_factor), 2) * ($coverage / 1000)) + $annaul_policy_fee;

                    $semi_annual = (round(($rate_from_db * $semi_annual_mode_factor), 2) * ($coverage / 1000)) + $semi_annual_policy_fee;


                    // $coverage = (int)$data['coverage'];
                    // $rate = $coverage / 1000;             //divided by 1000 it's fixed /mtlb company per 1000 Dollar k hissab s rate de rhi
                    // $rate = ($rate * $rate_from_db);
                    // $annual=($rate+$annaul_policy_fee)*$annual_mode_factor;

                    // $semi_annual = ($rate+$semi_annual_policy_fee)*$semi_annual_mode_factor;


                    // $monthly =($rate+$monthly_policy_fee)*$monthly_mode_factor;
